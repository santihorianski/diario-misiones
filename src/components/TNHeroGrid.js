      ...
      {displayArticles.map((article, idx) => {
        ...
        return (
          <Link key={article.id || idx} href={`/article/${article.id}`} className="group block relative w-full h-[450px] lg:h-[500px] overflow-hidden">
            ...
            <Image 
              src={(article.image || article.imageUrl || '').match(/\.(mp3|mp4|wav|avi)$/i) ? 'https://placehold.co/400x600/000b29/FFF?text=Noticia' : (article.image || article.imageUrl || 'https://placehold.co/400x600/000b29/FFF?text=Noticia')} 
              alt={article.title}
              fill
              className="object-cover transition-transform duration-700 group-hover:scale-110"
            />
            
            {/* Gradiente oscuro inferior más pronunciado para máxima legibilidad */}
            <div className="absolute inset-0 top-0 bg-gradient-to-t from-black via-black/80 to-transparent opacity-90 z-0 pointer-events-none"></div>

            {/* Contenido en la parte inferior (Subido con más padding bottom y más márgenes laterales) */}
            <div className="absolute bottom-0 left-0 right-0 px-8 pb-10 md:px-12 md:pb-14 pt-24 flex flex-col justify-end z-10 font-sans">
              
              {/* Badge de Categoría Dinámico */}
              <div className="mb-4">
                <span className={`${badgeColorClass} text-white text-[10px] md:text-[12px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-md shadow-md`}>
                  {categoryName}
                </span>
              </div>

              {/* Título de la Noticia (Sombra de texto fuerte para contrastar) */}
              <h2 className="!text-white font-sans text-[22px] md:text-[26px] font-extrabold leading-[1.25] tracking-tight mb-5 group-hover:!text-gray-200 transition-colors line-clamp-4 [text-shadow:_0_4px_12px_rgb(0_0_0_/_100%)]">
                {article.title}
              </h2>
