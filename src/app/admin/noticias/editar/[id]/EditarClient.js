...
import { ArrowLeft, Save, Search, Settings2, Image as ImageIcon } from 'lucide-react';
...
export default function EditarClient({ params, initialNews }) {
  const unwrappedParams = use(params);
  const { id } = unwrappedParams;
  const [isPending, startTransition] = useTransition();
  const router = useRouter();
  
  const [content, setContent] = useState(initialNews?.editedContent || initialNews?.fullContent || initialNews?.contentSnippet || '');
  const [imageUrl, setImageUrl] = useState(initialNews?.image || '');
  const [status, setStatus] = useState(initialNews?.status || 'published');
  
  const initialCats = initialNews?.categories || ['Destacado'];
  ...
  const handleSubmit = (e) => {
    ...
    startTransition(async () => {
      try {
        await updateArticleAction(id, formData);
        toast.success(status === 'published' ? 'Actualizado' : 'Borrador guardado');
        ...
        <div className="flex-1 space-y-8">
          ...
          <div className="space-y-6">
            <input 
              ...
              name="title" 
              required
              defaultValue={initialNews?.editedTitle || initialNews?.title || ''}
        ...
        <div className="lg:w-[320px] shrink-0 border-l border-neutral-800 pl-0 lg:pl-10 space-y-8">
          ...
          <div className="space-y-3">
            ...
            <textarea 
              name="seoDescription" 
              rows="4"
              defaultValue={initialNews?.seoDescription || ''} 
